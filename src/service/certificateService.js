...
const certificateService = {
  addCertificate: async ({ id, user_id, date, agency }) => {
    ...
    const newCertificate = { id, user_id, date, agency };
    const creatednewCertificate = await Certificate.create({ newCertificate });
    return creatednewCertificate;
  },

  getUserCertificateInfo: async ({ user_id }) => {
    const certificates = await Certificate.findOne({ user_id });
  ...
  deleteCertificate: async ({ _id }) => {
    const certificate = await Certificate.Model.findByIdAndDelete({ _id });
