...
const educationService = {
  addEducation: async ({
    ...
    const newEducation = { id, user_id, school_name, major, graduate_status };
    const creatednewEducation = await Education.create({ newEducation });
    return creatednewEducation;
  },

  getUserEducationInfo: async ({ user_id }) => {
    const educations = await Education.findOne({ user_id });
  ...
  deleteEducation: async ({ _id }) => {
    const education = await Education.Model.findByIdAndDelete({ _id });
